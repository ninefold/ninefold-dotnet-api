...
{
    public class ListIsos : ICommand
    {
        ...
        public ListIsosRequest Parameters { get; set; }

        public ListIsos(string apiKey, string secret, string baseUri, IComputeCommandAuthenticator authenticator, IComputeRequestBuilder builder)
        {
            _apiKey = apiKey;
            _secret = secret;
            _baseUri = baseUri;
        ...
        {
            var response = new ListIsosResponse();
            ...
            if ((responseStream != null) && (responseStream.CanRead))
            {
                ...
                response.Isos =
                    responseDocument.Root.Elements()
                        .Where(e => e.Name.LocalName.Equals("iso", StringComparison.InvariantCultureIgnoreCase))
                        .Select(Machine.From);
            }
